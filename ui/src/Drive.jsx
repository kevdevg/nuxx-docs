  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
            ...
            {projects.map(p => (
              <Link 
                ...
                style={{
                  ...
                  borderRadius: '8px',
                  textDecoration: 'none',
                  color: p.id === projectId ? 'white' : 'var(--text-primary)',
                  background: p.id === projectId ? 'var(--accent)' : 'transparent',
                  display: 'flex',
                  alignItems: 'center',
                  gap: '8px',
                  transition: 'all 0.2s'
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {/* Topbar */}
        <div style={{ padding: '24px 40px', borderBottom: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--glass-bg-darker)', backdropFilter: 'blur(12px)' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              {!currentProject?.gdrive_folder_id ? (
                linkingDrive ? (
                  ...
                  <button onClick={() => setLinkingDrive(true)} className="btn-secondary" style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px' }}>
                    <Cloud size={16} color="var(--accent)" /> Vincular Drive
                  </button>
                )
              ) : (
          ...
          {!projectId ? (
            ...
            <div className="animate-fade-in">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '20px' }}>
                ...
                {/* Folders */}
                {folders.map(f => (
                  <Link key={f.id} to={`/project/${projectId}/folder/${f.id}`} className="glass-panel" style={{ padding: '20px', textDecoration: 'none', color: 'var(--text-primary)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '12px', cursor: 'pointer', transition: 'transform 0.2s', background: 'var(--glass-bg-hover)' }} onMouseOver={e => e.currentTarget.style.transform = 'translateY(-4px)'} onMouseOut={e => e.currentTarget.style.transform = 'translateY(0)'}>
